...
source_data = spark.table(f"{catalog_name}.{schema_name}.source_data_ma")

# Get max update timestamps from existing data
max_train_timestamp = spark.table(f"{catalog_name}.{schema_name}.train_set_ma") \
    .select(F.max("update_timestamp_utc").alias("max_update_timestamp")) \
    .collect()[0]["max_update_timestamp"]

max_test_timestamp = spark.table(f"{catalog_name}.{schema_name}.test_set_ma") \
    .select(F.max("update_timestamp_utc").alias("max_update_timestamp")) \
    .collect()[0]["max_update_timestamp"]
...
new_data_train.write.mode("append").saveAsTable(f"{catalog_name}.{schema_name}.train_set_ma")
new_data_test.write.mode("append").saveAsTable(f"{catalog_name}.{schema_name}.test_set_ma")
